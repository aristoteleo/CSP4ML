...
def moments(
    ...
    layers_raw = [
        layer
        for layer in layers
        if not(layer.startswith("X")) and (not layer.endswith("matrix") and not layer.endswith("ambiguous"))
    ]
    layers_raw.sort(reverse=True)  # ensure we get M_us, M_tn, etc (instead of M_su or M_nt).
...
def calc_2nd_moment(X, Y, W, normalize_W=True, center=True, mX=None, mY=None):
    if normalize_W:
        if type(W) == np.ndarray:
            d = np.sum(W, 1).flatten()
        else:
