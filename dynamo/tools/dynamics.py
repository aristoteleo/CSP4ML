    ...
    for cur_grp_i, cur_grp in enumerate(_group):
        ...
        if assumption_mRNA.lower() == "ss" or (experiment_type.lower() in ["one-shot", "mix_std_stm"]):
            ...
            if type(params) == dict:
                ...
                beta = params.pop("beta") if "beta" in params else None
            ...
            if NTR_vel:
                ...
                if has_splicing:
                    if experiment_type == "kin":
                        ...
                        vel_S = vel.vel_s(U, S)
                        if len(beta.shape)>1:
                            beta = np.mean(beta,axis=1) # Saving cell wise beta will error later.
                        vel.parameters["beta"] = gamma
                        vel_N = vel.vel_u(U_)
                        vel_T = vel.vel_u(S)  # velocity_T = alpha - gamma_s * s (if gamma_s are counted instead of gamma_t)
                    elif experiment_type == "deg":
                        if splicing_labeling:
                            vel_U = np.nan
                            vel_S = vel.vel_s(U, S)
    ...
    if experiment_type.lower() == "kin":
        if est_method == "twostep":
            ...
            if has_splicing:
                ...
                (gamma_k,_,_,_,) = fit_slope_stochastic(S_smoothed, U_smoothed, US_smoothed, S2_smoothed, perc_left=None, perc_right=5)
