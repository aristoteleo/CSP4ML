...
def plot_new_RNA_counts_fitting_results(
    ...
    Args:
        ...
        cut_value: For better visualization, the x-axis is cut off to the given value.
    ...
    def _plot_bar_new_counts(labels, ob_counts, csp_counts, cszip_counts, number, min_len, gene_name, time, legend_flag,
                             p_CSP, p_CSZIP, figsize, dpi, cut_value, save_path):
        """"Use the bar chart to plot the observed counts and the line chart to plot the CSP and CSZIP model fits."""

        width = 0.5

        if cut_value:
            min_len = min(min_len, cut_value + 1)
            ob_counts = ob_counts[: min_len]
            csp_counts = csp_counts[: min_len]
            cszip_counts = cszip_counts[: min_len]
        ...
        if legend_flag:
            ...
            plt.text(0.74, 0.86, '$P_{CSP}=$' + ("%3.2f" % p_CSP) + '\n' + '$P_{CSZIP}=$' + ("%.2f" % p_CSZIP),
                     ha='center',
                     va='center', transform=ax.transAxes)

        plt.grid(False)
        if save_path:
            plt.savefig(f'{save_path}gene_{gene_name}_time{time}.pdf', dpi=dpi, ftransparent=True, bbox_inches="tight")
        plt.show()
    ...
    for j in range(len(time)):
        ...
        if j == 0:
            _plot_bar_new_counts(x_labels, f_obs_csp, f_exp_csp, f_exp_cszip, j, min_len, gene_name, time[j],
                                 True, p_value_csp, p_value_cszip, figsize, dpi, cut_value, save_path)
        else:
            _plot_bar_new_counts(x_labels, f_obs_csp, f_exp_csp, f_exp_cszip, j, min_len, gene_name, time[j],
                                 False, p_value_csp, p_value_cszip, figsize, dpi, cut_value, save_path)
...
def plot_total_RNA_counts_fitting_results(
    ...
    def plot_bar_total_counts(ob_counts, number, gene_name, time, legend_flag, p_value, figsize, dpi, save_path):
        ...
        if legend_flag:
            plt.legend()
            plt.text(0.75, 0.08, '$P=$' + ("%0.2f" % p_value), ha='center', va='center', transform=ax.transAxes)
        else:
